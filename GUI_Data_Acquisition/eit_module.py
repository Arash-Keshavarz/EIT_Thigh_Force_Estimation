...
class EITMeasurementModule:
    def __init__(self, parent):
        ...
        # Title Label
        title_label = ctk.CTkLabel(self.frame, text="EIT Measurement Config", font=("Arial", 20, "bold"), text_color="#3a3a3a")
        ...
        # Start Button
        self.start_button = ctk.CTkButton(self.frame, text="Start EIT Measurement", fg_color="#1F6F2b", hover_color="#166424", command=self.start_measurement)
    ...
    def create_parameter_inputs(self):
        ...
        for i, (label, default) in enumerate(params.items()):
            ...
            lbl = ctk.CTkLabel(self.frame, text=label, font=("Arial", 14), text_color="#3a3a3a")
            lbl.grid(row=i + 1, column=0, padx=10, pady=(10, 5), sticky="w")  
            entry = ctk.CTkEntry(self.frame, width=150, placeholder_text=default, border_color="#3a3a3a", fg_color="#f0f0f0")
            entry.configure(text_color="#3a3a3a")
